...
class GUIPrincipal:
            ...
            try:
                if order['type'] == 'buy':
                    ...
                    if current_price >= order['take_profit']:
                        ...
                        # Restablecer flag si ya no estamos en el límite
                        try:
                            max_orders = int(self.simulation_config.get('max_orders', 5))
                            if getattr(self, '_limit_orders_alerted', False) and len(self.active_orders) < max_orders:
                                self._limit_orders_alerted = False
                        except Exception:
                            pass
                        ...
                        # Si baja de límite, habilitar que el aviso vuelva a mostrarse al alcanzar el límite de nuevo
                        try:
                            max_orders = int(self.simulation_config.get('max_orders', 5))
                            if getattr(self, '_limit_orders_alerted', False) and len(self.active_orders) < max_orders:
                                self._limit_orders_alerted = False
    ...
    def _handle_candle_update(self, df_current):
        ...
        try:
            ...
            wait_candles = 0
            try:
                wait_candles = int(getattr(self, 'simulation_config', {}).get('wait_candles', 20))
            except Exception:
                wait_candles = 20
