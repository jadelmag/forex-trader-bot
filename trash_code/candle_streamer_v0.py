...
class CandleStreamer:
    ...
    def _notify_candle_update(self):
        ...
        try:
            ...
            if self.current_candle is not None:
                ...
                # Evitar FutureWarning: excluir entradas vacías antes del concat
                frames = [f for f in (df_current, cur) if f is not None and not f.empty]
                if frames:
                    df_current = pd.concat(frames)
    ...
    def _plot_last_candles(self):
        ...
        if self.current_candle is not None:
            ...
            # Evitar FutureWarning: excluir entradas vacías antes del concat
            frames = [f for f in (last_df, temp) if f is not None and not f.empty]
            if frames:
                last_df = pd.concat(frames)
