...
class CandleStrategies:
    ...
    def hammer_reversal_signal(self):
        """Patrón Hammer optimizado (reversión alcista) - Señal específica"""
        df = self.data.copy()
        ...
        df['Hammer_Signal'] = np.where(cond, 1, 0)
        return df[['Open','High','Low','Close','EMA20','Hammer_Signal']]

    def bullish_engulfing_signal(self):
        """Envolvente alcista tras tendencia bajista (optimizado) - Señal específica"""
        df = self.data.copy()
        ...
        df['Bullish_Engulfing_Signal'] = np.where(cond, 1, 0)
        return df[['Open', 'High', 'Low', 'Close', 'EMA20', 'Bullish_Engulfing_Signal']]

    def morning_star_signal(self):
        """Patrón Morning Star optimizado (reversión alcista) - Señal específica"""
        df = self.data.copy()
        ...
        df['Morning_Star_Signal'] = np.where(cond, 1, 0)
        return df[['Open','High','Low','Close','EMA50','Morning_Star_Signal']]

    # ---------------- Estrategias de reversión bajista ----------------
    def hanging_man_signal(self):
        """Patrón Hanging Man optimizado para detección de reversión bajista - Señal específica"""
        df = self.data.copy()
        ...
        df['Hanging_Man_Signal'] = np.where(cond, -1, 0)
        return df[['Open','High','Low','Close','EMA20','Hanging_Man_Signal']]

    def bearish_engulfing_signal(self):
        """Patrón Bearish Engulfing optimizado (reversión bajista) - Señal específica"""
        df = self.data.copy()
        ...
        df['Bearish_Engulfing_Signal'] = np.where(cond, -1, 0)
        return df[['Open','High','Low','Close','EMA20','Bearish_Engulfing_Signal']]

    def evening_star_signal(self):
        """Patrón Evening Star optimizado con confirmación de tendencia alcista previa - Señal específica"""
        df = self.data.copy()
        ...
        df['Evening_Star_Signal'] = np.where(cond, -1, 0)
        return df[['Open', 'High', 'Low', 'Close', 'EMA50', 'Evening_Star_Signal']]

    # ---------------- Estrategias de indecisión / continuación ----------------
    def doji_analysis(self):
        """Detección optimizada de velas Doji e indecisión - Análisis específico"""
        df = self.data.copy()
        ...
        df.loc[cond_bear_risk, 'Doji_Signal'] = -2
        df.loc[cond_bull_risk, 'Doji_Signal'] = 2

        return df[['Open','High','Low','Close','DojiType','Doji_Signal']]

    def marubozu_analysis(self):
        """Detección optimizada de velas Marubozu (alcista/bajista) - Análisis específico"""
        df = self.data.copy()
        ...
        df.loc[cond_bull, 'Marubozu_Signal'] = 1
        df.loc[cond_bear, 'Marubozu_Signal'] = -1

        return df[['Open','High','Low','Close','Marubozu_Signal']]

    # ---------------- Estrategias de múltiples velas ----------------
    def three_white_soldiers_signal(self):
        df = self.patterns.three_white_soldiers_pattern()  # Nombre corregido
        return df

    def three_black_crows_signal(self):
        df = self.patterns.three_black_crows_pattern()  # Nombre corregido
        return df

    # ---------------- Estrategias de trading combinadas ----------------
    def scalping_reversal_strategy(self):
        """Scalping con Hammer + Bullish Engulfing optimizado sobre soportes"""
        df_h = self.hammer_reversal_signal()
        df_e = self.bullish_engulfing_signal()
        df = self.data.copy()
        ...
        cond_h = (df_h['Hammer_Signal'] == 1) & (df['Close'] <= df['Support'] * 1.01)
        cond_e = (df_e['Bullish_Engulfing_Signal'] == 1) & (df['Close'] <= df['Support'] * 1.01)

        # Señal final
        df['Scalping_Signal'] = np.where(cond_h | cond_e, 1, 0)

        # Limpiar columna de soporte temporal
        df.drop(columns=['Support'], inplace=True)

        return df[['Open','High','Low','Close','Scalping_Signal']]

    def swing_trading_strategy(self):
        """Swing trading con Morning Star y Evening Star optimizado"""
        df_m = self.morning_star_signal()
        df_e = self.evening_star_signal()
        df = self.data.copy()

        # Señal alcista si Morning Star detectada
        cond_m = df_m['Morning_Star_Signal'] == 1

        # Señal bajista si Evening Star detectada
        cond_e = df_e['Evening_Star_Signal'] == -1

        # Señal final
        df['Swing_Signal'] = np.where(cond_m, 1, np.where(cond_e, -1, 0))

        # Mantener solo columnas relevantes
        return df[['Open','High','Low','Close','Swing_Signal']]

    def filter_with_trend_strategy(self):
        ...
        df['Trend_Filtered_Signal'] = np.where(cond_long, 1, np.where(cond_short, -1, 0))

        # Mantener solo columnas esenciales
        return df[['Open','High','Low','Close','EMA50','Trend_Filtered_Signal']]

    def stop_loss_take_profit_calculation(self, rr_ratio=2):
    ...
    def get_all_signals(self):
        """Obtiene todas las señales individuales en un solo DataFrame para análisis combinado"""
        df = self.data.copy()
        
        # Obtener todas las señales individuales
        signals = [
            self.hammer_reversal_signal()[['Hammer_Signal']],
            self.bullish_engulfing_signal()[['Bullish_Engulfing_Signal']],
            self.morning_star_signal()[['Morning_Star_Signal']],
            self.hanging_man_signal()[['Hanging_Man_Signal']],
            self.bearish_engulfing_signal()[['Bearish_Engulfing_Signal']],
            self.evening_star_signal()[['Evening_Star_Signal']],
            self.doji_analysis()[['Doji_Signal']],
            self.marubozu_analysis()[['Marubozu_Signal']],
            self.three_white_soldiers_signal()[['Three_White_Soldiers_Signal']],
            self.three_black_crows_signal()[['Three_Black_Crows_Signal']]
        ]
        
        # Combinar todas las señales
        for signal_df in signals:
